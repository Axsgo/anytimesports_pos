...
class ReportTax(models.AbstractModel):
    ...
    def get_lines(self, options):
        ...
        for tax in self.env['account.tax'].search(
                [('type_tax_use', '!=', 'none'),('active','=',True),('company_id','=',self.env.company.id)]):
            if tax.children_tax_ids:
                for child in tax.children_tax_ids:
                    if child.type_tax_use != 'none':
                        continue
                    taxes[child.id] = {'tax': 0, 'net': 0, 'name': child.name,
                                       'type': tax.type_tax_use, 'report_name': child.report_name if child.report_name else child.name}
            else:
                taxes[tax.id] = {'tax': 0, 'net': 0, 'name': tax.name,
                                 'type': tax.type_tax_use, 'report_name': tax.report_name if tax.report_name else tax.name}
