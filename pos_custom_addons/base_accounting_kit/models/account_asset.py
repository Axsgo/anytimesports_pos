...
class AccountAssetCategory(models.Model):
    ...
    account_profit_id = fields.Many2one('account.account',string="Asset Profit Account",required=True, domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
                                        help="Profit Account: To record the value of the sold asset with profit")
    account_loss_id = fields.Many2one('account.account',string="Asset Loss Account",required=True, domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
                                        help="Loss Account: To record the value of the sold asset with losses")
...
class AccountAssetAsset(models.Model):
    ...
    type = fields.Selection(related="category_id.type", string='Type',
                            required=True)
    sold_value = fields.Float(string="Sold Value",digits=0,help="Sell or Disposal Value of the asset.")
    ...
    def compute_generated_entries(self, date, asset_type=None):
        ...
        ungrouped_assets = self.env['account.asset.asset'].search(
            type_domain + [('state', '=', 'open'),('company_id','=',self.env.company.id),
    ...
    def set_to_close(self):
        if self.sold_value <= 0:
            raise UserError("Warning!!, Sold Value should be greater than zero.")
    ...
    def entry_close(self):
        if self.state == 'open' and self.value_residual == 0:
            self.depreciation_line_ids.post_lines_and_close_asset()
        else:
            raise UserError("Warning!!, Kindly Sell or Dispose before close the asset.")
...
class AccountAssetDepreciationLine(models.Model):
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            sold_amount = current_currency.with_context(
                date=depreciation_date).compute(line.asset_id.sold_value, company_currency)
            ...
            if sold_amount > 0:
                move_line_1 = {
                    ...
                    'debit': 0.0 if float_compare(sold_amount, 0.0,
                                                  precision_digits=prec) > 0 else -amount,
                    'credit': sold_amount if float_compare(sold_amount, 0.0,
            ...
            if sold_amount > 0:
                if sold_amount > amount:
                    move_line_3 = {
                        'name': asset_name,
                        'account_id': category_id.account_profit_id.id,
                        'credit': 0.0 if float_compare((sold_amount - amount), 0.0,
                                                       precision_digits=prec) > 0 else -amount,
                        'debit': (sold_amount - amount) if float_compare((sold_amount-amount), 0.0,
                                                         precision_digits=prec) > 0 else 0.0,
                        'journal_id': category_id.journal_id.id,
                        'partner_id': partner.id,
                        'analytic_account_id': category_id.account_analytic_id.id if category_id.type == 'purchase' else False,
                ...
                elif sold_amount < amount:
                    move_line_3 = {
                        'name': asset_name,
                        'account_id': category_id.account_loss_id.id,
                        'debit': 0.0 if float_compare((amount-sold_amount), 0.0,
                                                       precision_digits=prec) > 0 else -amount,
                        'credit': (amount - sold_amount) if float_compare((amount-sold_amount), 0.0,
                                                         precision_digits=prec) > 0 else 0.0,
                        'journal_id': category_id.journal_id.id,
                        'partner_id': partner.id,
                        'analytic_account_id': category_id.account_analytic_id.id if category_id.type == 'purchase' else False,
